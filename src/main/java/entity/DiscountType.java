...
public class DiscountType implements Serializable
{
    ...
    @OneToOne
    DiscountFixed fd = new DiscountFixed();
    
    @OneToOne
    DiscountQuantity qd = new DiscountQuantity();
    ...
    public double calcDiscount(double pricePerItem, int quantity)
    {
      ...
      if( quantity > 2 )  
      {
          return qd.calcDiscount( pricePerItem, quantity );
      }
      
      else
      {
          return fd.calcDiscount( pricePerItem, quantity );
      }
